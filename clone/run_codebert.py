...
from models import GraphBertModel, CodeBertModel
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss, logits = model(inputs_ids_1, position_idx_1, inputs_ids_2, position_idx_2,
                                 labels)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        (inputs_ids_1, position_idx_1, attn_mask_1,
         ...
         labels) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            lm_loss, logit = model(inputs_ids_1, position_idx_1, inputs_ids_2, position_idx_2,
                                   labels)
